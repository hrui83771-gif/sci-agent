...
function getCurrentDefaultEnterprise() {
  return {
    id: 'ZJ-JQZN-001',
    name: locale.value === 'en' ? 'Zhejiang Machine Intelligence Co., Ltd.' : '浙江机器智能公司',
    industry: locale.value === 'en' ? 'Intelligent Manufacturing / Machine Vision' : '智能制造 / 机器视觉',
    founded: '2019',
    scores: {
      tech: 92,
      team: 89,
...
if (!enterpriseStore.current) {
  enterpriseStore.setMock(getCurrentDefaultEnterprise())
}
...
@media (max-width: 720px) {
  ...
  .shell-toolbar {
    ...
    padding-bottom: 8px;
    background: linear-gradient(180deg, rgba(239, 245, 244, 0.96), rgba(239, 245, 244, 0.78));
    backdrop-filter: blur(12px);
  }

  .toolbar-group.mobile-only {
    flex: 1;
    min-width: 0;
  }
