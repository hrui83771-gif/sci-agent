...
function buildPyramid() {
  ...
  parts.forEach((p) => {
    ...
    // Custom geometry for a frustum (truncated pyramid) layer
    const partGeo = new THREE.BufferGeometry();
    
    const wBottom = (size / 2) * (1 - p.yStart / height);
    const wTop = (size / 2) * (1 - p.yEnd / height);
    const yB = p.yStart;
    const yT = p.yEnd;
    
    // 8 vertices for a frustum
    const v = [
      // Bottom 4
      [-wBottom, yB, wBottom], [wBottom, yB, wBottom], [wBottom, yB, -wBottom], [-wBottom, yB, -wBottom],
      // Top 4
      [-wTop, yT, wTop], [wTop, yT, wTop], [wTop, yT, -wTop], [-wTop, yT, -wTop]
    ];
    
    let vertices;
    
    if (wTop === 0) {
      // Top layer is a pyramid (5 vertices needed, but we use 5 for faces)
      const topV = [0, yT, 0];
      ...
      // Middle/Bottom layers are frustums
      vertices = new Float32Array([
        // Bottom face
        ...v[0], ...v[1], ...v[2],
        ...v[0], ...v[2], ...v[3],
    ...
    partGeo.setAttribute('position', new THREE.BufferAttribute(vertices, 3));
  ...
  for(let i=0; i<pyramidParticleCount; i++) {
    ...
    while(true) {
      ...
      const widthAtY = (size / 2) * (1 - y / height);
...
function animate() {
  ...
  if (autoRotate) {
    ...
    if (focusDim.value && screenFormationProgress > 0.5) {
      // Look straight forward (Z axis)
      spherical.theta += (0 - spherical.theta) * 0.1;
      spherical.phi += (Math.PI / 2.2 - spherical.phi) * 0.1;
    }
  }
  ...
  if (focusDim.value && pyramidBlocks[focusDim.value]) {
    ...
    // Lock the camera angle to face the screen directly
    spherical.theta += (0 - spherical.theta) * 0.05;
    spherical.phi += (Math.PI / 2.0 - spherical.phi) * 0.05; 
