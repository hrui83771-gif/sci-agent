    ...
    <div class="cube-shell" :class="{ 'is-building': buildPhase }">
      ...
      <div class="cube" :style="cubeStyle">
        ...
        >
          ...
          <ul class="face-facts">
            <li v-for="item in face.facts" :key="item">{{ item }}</li>
          </ul>
          <div class="face-bar">
        ...
        <div class="cube-face cube-face--info face-bottom">
          <span class="info-kicker">客户贷款需求</span>
          <strong>{{ enterprise.loanDemand?.amount || enterprise.scores.total }}</strong>
          <small>{{ enterprise.loanDemand?.summary || `${enterprise.city || ''} ${enterprise.founded || ''}` }}</small>
        </div>
      </div>
...
const cubeSize = computed(() => Math.max(240, Math.min(360, 270 + (totalScore.value - 60) * 2.3)))
...
function scoreState(score = 0) {
  if (score >= 88) return '优势'
  if (score >= 75) return '稳健'
  return '关注'
}
...
const clickableFaces = computed(() => {
  ...
  return mapping.map((item) => {
    ...
    return {
      ...
      facts: Array.isArray(detail.facts) ? detail.facts.slice(0, 5) : [],
...
.cube-shadow {
  ...
  transform: translateY(calc(var(--cube-size, 260px) * 0.58));
  filter: blur(18px);
}
